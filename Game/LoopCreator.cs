...
namespace Game
{
    internal class LoopCreator
    {
        internal static Action BuildLoop(Configuration config)
        {
            ...
            IRectangleBufferUnitRenderer unitRenderer = new RectangleBufferUnitRenderer();
            IRectangleUnitConverter rectangleUnitConverter = new RectangleUnitConverter(bufferObjectFactory);
            ICamera camera = new Camera(config.Resolution.AspectRatio);
            IAlphaTester alphaTester = new AlphaTester();
            // info...
            IFontMapper fontMapper = new FontMapper(textureCache, "font");
            IFontRenderer fontRenderer = new FontRenderer(surfaceRectangleBuilder.CreateRectangle(0, 0, 0.05f, 0.05f), polygonRenderer, worldTranslator, textureChanger, matrixManager,  0.05);
            ...
            var poly = surfaceRectangleBuilder.CreateRectangle(0.4, 0, 0.2f, 0.2f);
            var rectangle = rectangleUnitConverter.Convert(poly);
            ITexture man = textureCache.LoadTexture("opa proto.png");
            return () =>
            {
                while(!pressedKeyDetector.IsKeyDown(Keys.Escape))
                {
                    ...
                    alphaTester.Begin();
                    textureChanger.SetTexture(man.TextureId, 0);
                    unitRenderer.RenderUnit(rectangle);
                    alphaTester.End();
